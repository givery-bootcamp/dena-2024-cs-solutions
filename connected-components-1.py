...
# 全くスライドの DFS をいじっていないパターン（work を使っていないパターン）
# https://ideone.com/061FXl
...
def main(lines):
    ...
    for node_id in range(0, n):
        if color[node_id] == -1: # まだ塗られていなければ
            visited = DFS(graph, node_id, lambda x : 0)
            for visited_node, is_visited in enumerate(visited):
                if is_visited:
                    color[visited_node] = node_id
